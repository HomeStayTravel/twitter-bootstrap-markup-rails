module Twitter::Bootstrap::Markup::Rails::Components
  class FormBuilder < ActionView::Helpers::FormBuilder
    ...
    def telephone_with_country_code_field(method, country_codes, options = {})
      ...
      country_code = obj.try(country_code_method)

      country_code_select = @template.select(object_name, country_code_method, @template.options_for_select(country_codes, selected: country_code), options.merge(include_blank: "Prefix"), {:class => 'telephone_field_country_code'})

      telephone_number_method = "#{method}_number"

      obj = @object || @template.instance_variable_get("@#{object_name}")
      phone_number = obj.try(telephone_number_method)
